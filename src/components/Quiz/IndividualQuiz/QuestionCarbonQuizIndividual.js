...
const Tittle = styled.div`
	...
	color: #38a66d;
	padding-bottom: ${(props) => (props.id === 0 && !props.simpleQuiz ? '2%' : '0%')};
`;
...
const SimpleQuizImage = styled.img`
	width: 13em;
	display: ${(props) => ((props.id === 0 && !props.simpleQuiz) || props.id === 6 ? 'none' : 'initial')};
...
export const QuestionCarbonQuizIndividual = ({
	...
	const { id, title, description, image2, ComplexForm, ComplexFormState } = rowsValues;
		...
		<QuestionsWrapper id={id}>
			...
			<InputBox id={id}>
				{simpleQuiz && ComplexForm ? (
					<QuestionQuizIndividualSimple rowsValues={rowsValues} handleSliderInput={handleSliderInput} />
				) : (
